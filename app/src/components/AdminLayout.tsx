...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-dvh bg-ink-50 md:flex">
      {/* 亮橘側欄（桌機） */}
      <aside className="hidden md:flex w-72 shrink-0 flex-col min-h-[100dvh] bg-accent-700 text-white sticky top-0 h-[100dvh]">
      ...
      <div className="flex-1 min-w-0">
        {/* 手機頂部列 */}
        <header className="md:hidden sticky top-0 z-20 bg-accent-700 text-white px-4 py-3 flex items-center justify-between">
        ...
        {/* 手機橫向導覽 */}
        <nav className="md:hidden sticky top-[48px] z-20 bg-accent-700 text-white px-3 py-2 flex gap-1.5 overflow-x-auto">
          {ADMIN_NAV.map((n) => (
            <NavLink
              key={n.to}
              to={n.to}
