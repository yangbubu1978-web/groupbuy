-- 修正：admin Edge Function 以 service_role 代理呼叫時，
-- auth.uid() 不會帶入原始管理員身份，不能因此被誤判 forbidden。
-- Edge Function 已先以使用者 JWT 驗證 admins；service_role 僅由受保護的 Edge Function 使用。
